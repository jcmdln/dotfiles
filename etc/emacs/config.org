...
reaching 80% of the current heap size, GC will not occur.  After init
completes we will set this back to the default.
...
the total number of GC pauses we experience.  At this size, most
platforms should only notice a performance improvement without
